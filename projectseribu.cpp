...
void inputMultiLine(char* buffer) {
    ...
    while (true) {
        ...
        if (strlen(buffer) + strlen(line) + 2 < PanjangHal) {
            ...
            cout << "Batas karakter halaman terlampaui!\n";
            break;
        }
    }
}
...
void cariBuku() {
	...
	// Panggil fungsi sortir agar daftar buku selalu terurut saat dicari
    selectionSortBukuByJudul();
    
    char judulCari[100] = "";
    cin.ignore();
    cout << "Masukkan judul buku untuk dicari (kosongkan untuk melewati): ";
    cin.getline(judulCari, 100);
    
    cout << "\nHasil Pencarian:\n";
    bool ditemukan = false;
    for (int i = 0; i < jumlahBuku; i++) {
        // strstr untuk mencari substring, tolower untuk case-insensitive (opsional)
        if (strstr(daftarBuku[i].judul, judulCari) != NULL) {
            cout << i+1 << ". " << daftarBuku[i].judul
                 << " (" << daftarBuku[i].jumlahHalaman << " halaman)\n";
            ditemukan = true;
        }
